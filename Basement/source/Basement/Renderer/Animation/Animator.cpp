...
namespace Basement
{
	...
	void Animator::SolveInverseKinematicsCCD(const glm::vec3& targetPosition)
	{
		...
		for (int i = 0; i < endEffectorCount - 1; ++i)
		{
			...
			int parentId = boneDataMap[m_EndEffectors[i + 1]->name].Id;
			glm::vec3 endPosition = glm::inverse(m_GlobalBoneMatrices[parentId]) * glm::vec4(targetPosition, 1.0f);
			...
			if (cosine < 1.0f)
			{
				...
				// rotate
				rotationMatrix = glm::mat4(rotation) * toLeafTransform;	// to leaf space
			}
			...
			auto bone = m_CurrentAnimation->FindBone(m_EndEffectors[i]->name);
			BM_CORE_INFO("rotationMatrix: " + glm::to_string(rotationMatrix));
			bone->SetLocalTransformation(rotationMatrix);
			m_EndEffectors[i]->localTransformation = rotationMatrix;
			...
			for (int k = i; k >= 0; --k)
			{
				...
				//auto localMat = currentEndEffector->localTransformation;
				auto localMat = m_CurrentAnimation->FindBone(currentEndEffector->name)->GetLocalTransformation();;
				auto globalMat = m_GlobalBoneMatrices[parentIndex] * localMat;
				m_GlobalBoneMatrices[currentIndex] = globalMat;
