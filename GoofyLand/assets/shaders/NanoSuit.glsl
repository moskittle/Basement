...
{
    // ambient
    vec3 ambient = u_Light.ambient * texture(material.texture_diffuse1, v_TexCoord).rgb;

    // diffuse
    vec3 normal = normalize(v_Normal);
    vec3 lightDir = normalize(u_Light.position - v_FragPosition);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = u_Light.diffuse * diff * texture(material.texture_diffuse1, v_TexCoord).rgb;
